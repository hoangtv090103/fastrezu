...
CREATE POLICY "Authenticated users can upload their own profile photo" ON storage.objects FOR INSERT
WITH
    CHECK (
...
CREATE POLICY "Anyone can read profile photos" ON storage.objects FOR
SELECT USING (bucket_id = 'profile-photos');

-- Allow authenticated users to overwrite (upsert) their own photo
CREATE POLICY "Authenticated users can update their own profile photo" ON storage.objects
FOR UPDATE
    USING (
        bucket_id = 'profile-photos'
        AND auth.uid ()::text = (storage.foldername (name)) [1]
...
CREATE POLICY "Authenticated users can delete their own profile photo" ON storage.objects FOR DELETE USING (
    bucket_id = 'profile-photos'
    AND auth.uid ()::text = (storage.foldername (name)) [1]
);

-- Note: storage.objects permissions are managed via Supabase RLS policies defined above.
-- Dangerous GRANT ALL statements have been removed to prevent authenticated users from modifying bucket configs.